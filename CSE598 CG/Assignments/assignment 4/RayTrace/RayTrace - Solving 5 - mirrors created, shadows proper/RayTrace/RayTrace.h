...
public:
	...
	void RayTrace1(Vector ray, int r_depth)
	{
		...
		for (int i = 0; i < m_Scene.GetNumObjects(); i++)  //drawing loop
		{
						...
						for (int j = 0; j < m_Scene.GetNumLights(); j++)
						{
							if ((lightflag[0] == 1 && j == 0) || (lightflag[1] == 1 && (j == 1 || j == 2)))
							{
								...
								if (depth <= zdepth && !shadowflag)
								{
									...
									color = color1 + Ia + (Id + Is)*(!shadowflag);
									zdepth = depth;
								}
			...
			if (scenematerial.reflective.Magnitude() > 0.0) //atleast one of the color has reflective characteristics
			{
				...
				if (r_depth < depth_limit)
				{
					...
					Vector reflectedray = reflray - (IP + reflray * epsilon);
					/*RayTrace1(reflectedray, r_depth + 1) ;
					color = color * scenematerial.reflective;
*/
				}
	...
	{
		zdepth = 10000.0; //initial depth magnitude ~ infinity.
		int r_depth = 0;
		//int static temp = 1;
		...
		RayTrace1(screen_pos-m_Camera.position,1);
