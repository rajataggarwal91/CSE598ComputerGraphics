...
void keybd_event1(unsigned char cAscii, int nKey, int idonknowwhattodowoththisvariable )
{
	...
	if (cAscii == 'q')
	{
		...
		/*g_NormalRenderer.sceneCam.up.z = dist * cos(PI/2 + vangle * (PI / 180));
		g_NormalRenderer.sceneCam.up.y = dist* sin(PI/2 + vangle * (PI / 180));
		g_NormalRenderer.sceneCam.up.Normalize();
		*/g_RayTrace.m_Camera.target.z = -dist * cos(vangle * (PI / 180));
		g_RayTrace.m_Camera.target.y = -dist * sin(vangle * (PI / 180));
		/*g_RayTrace.m_Camera.up.z = dist * cos(PI/2 + vangle * (PI / 180));
		g_RayTrace.m_Camera.up.y = dist * sin(PI/2 + vangle * (PI / 180));
*/
...
int main (int argc, char ** argv)
{
	...
	cout <<"main : "<< g_RayTrace.m_Scene.GetNumObjects();
	glutInit(&argc,argv);
