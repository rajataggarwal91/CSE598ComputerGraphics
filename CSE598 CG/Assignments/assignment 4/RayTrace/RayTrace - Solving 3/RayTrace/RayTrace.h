			...
			//	_getch();
				...
				*/
				...
				scenematerial.diffuse=
					(
						((material[0].diffuse * ((IP - p1).Cross((p2 - IP)).Magnitude() ))
					+ (material[1].diffuse * ((IP - p2).Cross((p0 - IP)).Magnitude() ))
					+ (material[2].diffuse * ((IP - p0).Cross((p1 - IP)).Magnitude() )))
					//------------------------------------------------------------------------------------
								/  (((p0 - p1).Cross(p2-p0)).Magnitude())
					);

				scenematerial.specular =
					(
						((material[0].specular * ((IP - p1).Cross((p2 - IP)).Magnitude()))
					+ (material[1].specular * ((IP - p2).Cross((p0 - IP)).Magnitude()))
					+ (material[2].specular * ((IP - p0).Cross((p1 - IP)).Magnitude())))
					//------------------------------------------------------------------------------------
					/ (((p0 - p1).Cross(p2 - p0)).Magnitude())
					);

				/*cout << scenematerial.diffuse.x << scenematerial.diffuse.y<<scenematerial.diffuse.z;
				_getch();
