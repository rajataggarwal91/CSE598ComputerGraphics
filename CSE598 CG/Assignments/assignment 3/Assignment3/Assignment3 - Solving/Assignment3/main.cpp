...
//imported library for vector operations
#include "ImathVec.h"
#include <iostream>
#include "glsl.h"//library for glsl
...
//Q1 : Creating ground plane
	...
	virtual void OnInit()
	{
		...
		glShadeModel(GL_SMOOTH); //can be GL_FLAT or GL_SMOOTH, SMOOTH makes the interpolation per fragment.
		glEnable(GL_DEPTH_TEST); //??????????why is this exactly used?????????????????
